            ...
            @foreach ($cats as $cat)
                <div class="bg-[#d5c1db] rounded-lg shadow-md p-4 text-center">
                    ...
                    <div class="flex justify-center space-x-2 mt-4">
                        <form action="{{ route('cats.edit', $cat->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="btn btn-sm text-white bg-[#896a92] hover:bg-[#9c7fa6] transition-colors px-2 py-1 text-center">
                        ...
                        <form action="{{ route('cats.destroy', $cat->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm text-white bg-[#896a92] hover:bg-[#9c7fa6] transition-colors px-2 py-1 text-center" onclick="return confirm('¿Estás seguro de que quieres eliminar este gato?')">
